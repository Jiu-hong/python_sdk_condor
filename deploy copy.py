...
class DeployHeader:
    ...
    def serialize(self):
        ...
        deployhash = self.byteHash(s_account, s_time_now, s_ttl, s_gas_price,
                                   s_body_hash, s_dependencies, s_chain_name)
        return deployhash

    def byteHash(self, s_account, s_time_now, s_ttl, s_gas_price,
                 s_body_hash, s_dependencies, s_chain_name):

        h = blake2b(digest_size=32)
        s_account = bytes.fromhex(s_account)
        s_time_now = bytes.fromhex(s_time_now)
        s_ttl = bytes.fromhex(s_ttl)  # ok
        s_gas_price = bytes.fromhex(s_gas_price)
        s_body_hash = bytes.fromhex(s_body_hash)

        s_dependencies = bytes.fromhex(s_dependencies)

        s_chain_name = bytes.fromhex(s_chain_name)

        h.update(s_account + s_time_now + s_ttl + s_gas_price +
                 s_body_hash + s_dependencies + s_chain_name)
        # h.update(s_account + s_time_now + s_ttl + s_gas_price +
        #          s_body_hash + s_chain_name)
        return h.hexdigest()
